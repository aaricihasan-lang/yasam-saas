...
**Son güncelleme:** 2026-07-20 (S2.17 kapanış — feature branch)
...
- **S2.17 feature kod** `1ab8601` (`feat(yasam-hafizasi): add S2.17 search_tsv query plan`) + son main sync merge `46fd460` — **yalnız `work/yh-s2-17` feature branch'te; main'e merge EDİLMEDİ; push/PR yok.**
...
- **S2.17:** ❌ **Push YAPILMADI.** Feature branch `work/yh-s2-17` yalnız yerelde (remote feature branch yok); PR açılmadı. Push/PR ayrı onayla yapılacak.
...
- **Aktif geliştirme YOK.** S2.17 feature branch'te tamamlandı (yukarıda); **push/PR bekliyor.** Sıradaki **S2.18 mimari analiz bekliyor** (henüz açılmadı, kod yok). S2.18 beklenen kapsam (S2.17'ye karıştırılmaz): DB Execution/Adapter (`.textSearch("search_tsv", tsquery, {config:"simple"})`, type omit → `to_tsquery`) + `ts_rank`/aday tavan (`YH_CANDIDATE_LIMIT`) · gerçek Supabase dictionary/stone-exclusion adapter · tenant/visibility filtresi · Kanıt Kapısı · derece · "Neden?" · Retrieval Pipeline · INV harness.
- **Onay bekleyen:** (1) S2.17 feature branch **push + PR** onayı; (2) **S2.18** mimari analiz turu onayı. Her ikisi de **otomatik başlamaz**. Not: ROADMAP'te **S2.06 yoktur**.

## Bekleyen İşler

- Yaşam Hafızası **S2.17** push + PR (onayla) → **S2.18** (otomatik açılmaz): DB Execution/Adapter (`.textSearch("search_tsv", tsquery, {config:"simple"})`, type omit → `to_tsquery`) + `ts_rank`/aday tavan (`YH_CANDIDATE_LIMIT`) + gerçek Supabase dictionary/stone-exclusion adapter · tenant/global merge + `lang`/`is_active` filtreleri · [4] Kanıt Kapısı · [5] derece · [6] "Neden?" · INV harness. Her aşama ayrı salt-okunur analiz + kullanıcı onayıyla.
