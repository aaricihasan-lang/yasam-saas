...
**Son güncelleme:** 2026-07-20 (S2.18 açılış — yalnız doküman)
...
- **Yaşam Hafızası™ — Sprint 2 (Retrieval / Hızlı Tarama):** S2.01–S2.05 + S2.07 + **S2.08–S2.12 (PR #3, `555030a`)** + **S2.13 (PR #4, `4c672e9`)** + **S2.14 (PR #6)** + **S2.15 (PR #8, `f56ec60`)** + **S2.16 (PR #11, `dc94b98`)** + **S2.17 (search_tsv tsquery Plan): main'e MERGE EDİLDİ — PR #13, merge `7344b6d`** origin/main'de. `origin/main` sonrasında **`9bbe5da`**'ya ilerledi (yalnız Danışan Yolculuğu/clients mobil+Word UX; **YH-dışı drift**). **AKTİF: S2.18 — Saf Retrieval Query Descriptor / Execution Contract (EX-D)** açıldı (yalnız doküman; kod bekliyor) — `work/yh-s2-18` @ `9bbe5da`. Kilitli backlog (S2.19+): DB Execution/Adapter (`.textSearch`/`.rpc` + ts_rank/aday tavan) + PostgreSQL RPC/DDL → Evidence Gate → Ranking → Retrieval Pipeline → Search UI.
...
- **AKTİF: S2.18 — Saf Retrieval Query Descriptor / Execution Contract (EX-D)** — açıldı (yalnız doküman; kod/harness YOK). `work/yh-s2-18` @ `9bbe5da`. **Kapsam:** S2.17 `TsQueryPlan` çıktısını tüketen, **DB'siz/saf/deterministik**, retrieval execution niyetini **typed descriptor** olarak üreten builder. Kararlar K1–K7: K1 visibility yeniden uygulanmaz (S2.13 `VisibilityContext` taşınır) · K2 fail-closed discriminated union `kind:'noop'|'query'` (adapter yalnız `query` çalıştırır) · K3 string SQL üretilmez · K4 ranking intent taşınır fakat ts_rank yürütülmez · K5 config/SQL drift borcu S2.19'a · K6 immutability (freeze + 4 garanti) · K7 module facet kapsam dışı. Bkz. `CURRENT_TASK.md`. **Gerçek DB execution/RPC/DDL + Evidence Gate S2.19+ (kapsam dışı).**
- **Onay bekleyen:** **S2.18 kod aşaması** (`retrievalQuery.ts` + harness) — doküman açılışından sonra ayrı onayla. **Otomatik başlamaz.** Not: ROADMAP'te **S2.06 yoktur**.

## Bekleyen İşler

- Yaşam Hafızası **S2.18 kod** (`retrievalQuery.ts` + harness; onayla) → **S2.19** (otomatik açılmaz): gerçek DB Execution/Adapter (`.textSearch`/`.rpc`) + PostgreSQL weighted ts_rank **RPC/DDL** (Dashboard) + `YH_CANDIDATE_LIMIT` + gerçek Supabase dictionary/stone-exclusion adapter + **config/SQL drift çözümü** (RPC-param VEYA sync-harness) → **[4] Kanıt Kapısı (Evidence Gate)** → [5] derece → [6] "Neden?" → Retrieval Pipeline → INV harness. Her aşama ayrı salt-okunur analiz + kullanıcı onayıyla.
