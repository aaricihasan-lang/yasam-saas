...
| **S2.19-BF/BF-1A** | **`aromaterapi:oils` Dry-Run Pilot Driver** — yalnız dry-run, fail-closed, cursor-bazlı, resumable local Node driver + mock harness; auth header-bazlı env-only; sabitler compile-time; response `plannedInsert/update/unchanged` taşımaz | ✅ Kod-tam (`work/yh-bf1a`; `ba43d4a`; **push/PR YOK**) | `scripts/yh-oils-dryrun-driver.ts` + `scripts/yh-oils-dryrun-driver-harness.ts` (mock; **76/76**). Route/adapter/migration/BF-0 guard değişmedi. **Gerçek API/SQL/dry-run YOK** |
