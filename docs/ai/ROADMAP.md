...
| **S2.19-BF/BF-1A** | **`aromaterapi:oils` Dry-Run Pilot Driver** — yalnız dry-run, fail-closed, cursor-bazlı, resumable local Node driver + mock harness; auth header-bazlı env-only; sabitler compile-time; response `plannedInsert/update/unchanged` taşımaz | 🟡 Kod fazı (`work/yh-bf1a` @ `8a9eb2c`) | `scripts/yh-oils-dryrun-driver.ts` + `scripts/yh-oils-dryrun-driver-harness.ts` (mock). Route/adapter/migration değişmez. **Gerçek API/SQL/dry-run YOK** |
