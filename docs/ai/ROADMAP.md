...
| **S2.16** | [2b] Dictionary Expansion (saf/DB'siz; S2.15 query-origin çıktısını küratörlü snapshot ile additif synonym-origin `Concept`'lerle genişletir; çok-kelime = bitişik alt-dizi; tek-sıçrama/transitif yok) | 🟡 | `work/yh-s2-16` açıldı (docs; kod yok). Plan: `search/dictionaryExpansion.ts` + harness; `expandConcepts(base, normalizedText, entries): readonly Concept[]`; giriş `DictionaryEntry { canonical, synonyms[] }` |
| S2.17 | [3] `search_tsv` sorgu + tsquery-boyut korkuluğu · gerçek Supabase dictionary/stone-exclusion adapter · tenant/global merge + `lang`/`is_active` filtreleri · **Kanıt Kapısı** · derece · deterministik "Neden?" · INV-1/INV-2 harness | ⬜ | `10-roadmap.md` (S2.16 sonrası ayrı analiz/onay; **otomatik açılmaz**) |

> **S2.05, S2.07, S2.08–S2.12, S2.13, S2.14, S2.15 tamamlandı ve main'de** (PR #1 `cd9c77c`, PR #2 `2b19743`, PR #3 `555030a`, PR #4 `4c672e9`, **PR #6** `dd29167`, **PR #8** `404841b`/kod `f56ec60`; güncel `origin/main` = `77aa824`). **S2.16 (Dictionary Expansion) açıldı** — `work/yh-s2-16` (taban `77aa824`); S2.15 çıktısını additif genişleten sözlük/synonym katmanı ([2b]); kod yok, worktree + karar kilidi + açılış docs. Kilitli backlog: Dictionary Expansion → search_tsv → Stone Exclusion Adapter → Evidence Gate → Ranking → Retrieval Pipeline → Search UI. Sonrası (S2.17) ayrı analiz/onayla; **otomatik açılmaz**. ROADMAP'te **S2.06 yoktur**.
