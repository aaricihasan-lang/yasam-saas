...
| **S2.17** | [3] `search_tsv` tsquery PLANI (saf/DB'siz; `Concept[]` → güvenli deterministik tsquery; tek kelime prefix, çok kelime exact phrase, OR birleşim, config `simple`, lexeme allowlist re-assert; **DB/RPC/adapter/ts_rank/candidate-limit YOK**) | 🟢 Kod+doğrulama tamam / **PR bekliyor** (main'de DEĞİL) | `search/tsQueryPlan.ts` + `scripts/yh-tsquery-plan-harness.ts` (feature kod `1ab8601`, `work/yh-s2-17`; harness **57/57**); `buildTsQueryPlan(concepts: readonly Concept[]): TsQueryPlan`. Push/PR/main-merge YOK |
| **S2.18** | DB Execution/Adapter (`.textSearch("search_tsv", tsquery, {config:"simple"})`, type omit → `to_tsquery`) + `ts_rank`/aday tavan (`YH_CANDIDATE_LIMIT`) · gerçek Supabase dictionary/stone-exclusion adapter · tenant/visibility filtresi · **Kanıt Kapısı** · derece · deterministik "Neden?" · Retrieval Pipeline · INV-1/INV-2 harness | ⬜ | Sonraki sprint; `10-roadmap.md` (S2.17 sonrası ayrı mimari analiz/onay; **otomatik açılmaz**). S2.18 kapsamı S2.17'ye karıştırılmaz |

> **S2.05, S2.07, S2.08–S2.12, S2.13, S2.14, S2.15, S2.16 tamamlandı ve main'de** (PR #1 `cd9c77c`, PR #2 `2b19743`, PR #3 `555030a`, PR #4 `4c672e9`, **PR #6** `dd29167`, **PR #8** `404841b`/`f56ec60`, **PR #11** `dc94b98`/kod `2192f74`; güncel `origin/main` = `95efcab`). **S2.17 (search_tsv Query / tsquery Plan): kod + doğrulama TAMAMLANDI, feature branch commit'i hazır, PR bekliyor** — `work/yh-s2-17` (feature kod `1ab8601`, son main sync `46fd460`; harness 57/57 · tsc/eslint PASS). ⚠️ **main/production'a merge EDİLMEDİ; push/PR YOK.** Sonraki **S2.18** ayrı mimari analiz/onayla açılacak (henüz açılmadı). Backlog: DB Execution/Adapter (+ts_rank/aday tavan) → Stone Exclusion Adapter → Evidence Gate → Ranking → Retrieval Pipeline → Search UI. **Otomatik açılmaz.** ROADMAP'te **S2.06 yoktur**.
