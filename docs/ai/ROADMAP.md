...
| **S2.18** | [3-saf] **Retrieval Query Descriptor / Execution Contract (EX-D)** — S2.17 `TsQueryPlan` çıktısını tüketir; **DB'siz/saf/deterministik**; execution niyetini typed descriptor olarak üretir (config/column/tsquery + S2.13 `VisibilityContext` taşınır + ranking intent: `YH_TSV_WEIGHTS`/desc/`YH_CANDIDATE_LIMIT`); fail-closed `kind:'noop'\|'query'`; **string SQL YOK · ts_rank YOK · DB YOK · `invariantFilters`/`requireNonPii`/`excludeDemo` YOK** | ✅ Kod tamam (`ab1d5f5`; push/PR bekliyor) | `search/retrievalQuery.ts` + `scripts/yh-retrieval-query-harness.ts` (`work/yh-s2-18`; kod commit `ab1d5f5`; harness **52/52**); `buildRetrievalQuery(plan: TsQueryPlan, visibility: VisibilityContext): RetrievalQueryDescriptor`. Visibility kararı S2.13'te; WHERE/RPC + görünürlük→LIMIT 150 sırası S2.19. K1–K7 bkz. `CURRENT_TASK.md` |
| **S2.19+** | Gerçek DB Execution (`.textSearch`/`.rpc`) + PostgreSQL weighted ts_rank **RPC/DDL** (Dashboard) + config/SQL drift çözümü (RPC-param VEYA sync-harness) → **[4] Kanıt Kapısı (Evidence Gate)** → [5] derece → [6] "Neden?" → Retrieval Pipeline → INV-1/INV-2 harness | ⬜ | Ayrı mimari analiz/onay; **otomatik açılmaz**. Evidence Gate downstream (`Candidate[]` tüketir, `TsQueryPlan` değil) |

> **S2.05, S2.07, S2.08–S2.12, S2.13, S2.14, S2.15, S2.16, S2.17 tamamlandı ve main'de** (PR #1 `cd9c77c`, PR #2 `2b19743`, PR #3 `555030a`, PR #4 `4c672e9`, **PR #6** `dd29167`, **PR #8** `404841b`/`f56ec60`, **PR #11** `dc94b98`, **PR #13** `7344b6d` = S2.17). `origin/main` sonrasında **`9bbe5da`**'ya ilerledi (yalnız Danışan Yolculuğu/clients mobil+Word UX; **YH-dışı drift**). **S2.18 — Saf Retrieval Query Descriptor / Execution Contract (EX-D): kod TAMAMLANDI** (`work/yh-s2-18`; kod commit `ab1d5f5`; harness 52/52; **push/PR bekliyor, S2.19 başlamadı**). Gerçek DB execution/RPC/DDL + Evidence Gate **S2.19+** (kapsam dışı, otomatik açılmaz). ROADMAP'te **S2.06 yoktur**.
