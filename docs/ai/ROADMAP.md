...
| **S2.14** | Retrieval Türkçe metin normalizasyonu (saf/DB'siz; §1 simetrik normalize + tokenizasyon; stemmer/stop-list YOK) | ✅ | `search/normalize.ts` + `scripts/yh-normalize-harness.ts` (`dd29167`, `origin/work/yh-s2-14`; harness **83/83**; query–index simetrisi production SELECT ile doğrulandı; PR bekliyor) |
| S2.x | [2] sözlük/concept-set · [3] `search_tsv` sorgu · gerçek Supabase stone-exclusion adapter · **Kanıt Kapısı** · derece · deterministik "Neden?" · INV-1/INV-2 harness | ⬜ | `10-roadmap.md` (numaralandırma S2.14 sonrası ayrı analiz/onay; **otomatik açılmaz**) |

> **S2.05, S2.07, S2.08–S2.12, S2.13 tamamlandı ve main'de** (PR #1 `cd9c77c`, PR #2 `2b19743`, PR #3 `555030a`, **PR #4 `4c672e9`**). **S2.14 (Retrieval Türkçe Metin Normalizasyonu) tamamlandı** — `work/yh-s2-14` @ `ad03579`, kod commit `dd29167`, `origin/work/yh-s2-14`'e push edildi; saf normalize birimi ([1] boru hattı adımı); **PR bekliyor**. Sonrası: kalan S2.x işleri (numaralandırma ayrı analiz/onayla; otomatik açılmaz). ROADMAP'te **S2.06 yoktur**.
