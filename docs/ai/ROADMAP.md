...
| **S2.19-BF/BF-0** | **Kaynak PII Sınıflandırma Guard'ı** — 17 kaynağa zorunlu `classification` (**safe-non-pii 15 / pii 1 / unclassified 1 / deferred 0**); yalnız `safe-non-pii && enabled` indekslenir (fail-closed; dry-run + write); INV-PII ana index CHECK değişmez | ✅ Kod-tam (`work/yh-bf0`; `b69942f`; **push/PR YOK; production YOK**) | `indexer/sources.ts` (classification) + `sourceGuard.ts`(yeni) + `adminIndexRequest.ts` (403 `source-not-indexable`) + `indexSourcePage.ts` (son savunma) + harness (**39/39**). Migration/CHECK değişmedi; +5 test-config ripple. Driver/dry-run/write DIŞI |
| **S2.19-BF/BF-1A** | **`dogaltas:knowledge` Dry-Run Pilot Driver** — yalnız dry-run, fail-closed, cursor-bazlı, resumable local Node driver + mock harness; auth header-bazlı env-only; sabitler compile-time; response `plannedInsert/update/unchanged` taşımaz. (Pilot `aromaterapi:oils`→`dogaltas:knowledge`: aromaterapi aktif geliştirmede, stabil kaynağa taşındı) | ✅ Kod-tam (`work/yh-bf1a`; switch `c706ea2`/`75391a7`; **push/PR YOK**) | `scripts/yh-dogaltas-knowledge-dryrun-driver.ts` + `…-harness.ts` (mock; **78/78**). Route/adapter/migration/BF-0 guard değişmedi. **Gerçek API/SQL/dry-run YOK** |
| **S2.19-BF/BF-1B-FIX** | **Global Sentetik Tenant Guard** — `ADMIN_LIBRARY_TENANT_ID` gerçek kullanıcı/tenant DEĞİL (seed/template namespace); kayıtları indexlenemez, **shared'e çevrilemez**, writer'a ulaşamaz, retrieval'da görünemez (YH geneli invariant). UUID tek-kaynak `lib/tenancy/syntheticTenants.ts`; `excludedSynthetic` zorunlu metrik (demo'dan ayrı); writer fail-fast `synthetic-tenant-unit`; driver state v2 fail-closed | ✅ Kod-tam (`work/yh-bf1b-synthetic-guard`; **push/PR YOK**) | BF-1B production kontrolü: 239/239 satır sentetik → `dogaltas:knowledge` **gerçek backfill pilotu DEĞİL**; gerçek pilot ayrı onayla seçilecek. RPC/visibility/Doğaltaş API değişmedi; **production verisi değişmedi** |
| **S2.19-BF/BF-1C→D → BF-2 → S2.19C** | **1C:** canlı dry-run = **sentetik exclusion production smoke** (beklenen fetched=239 · excludedSynthetic=239 · eligible=0 · write=null) · **1D:** doğrulama · **BF-2:** write (ayrı hard gate; önce gerçek pilot kaynak seçimi) · **S2.19C:** canlı smoke + INV | ⬜ | Ayrı onay; **otomatik açılmaz**. "Tam güvenli canlı retrieval" S2.19C öncesi ilan edilmez |
