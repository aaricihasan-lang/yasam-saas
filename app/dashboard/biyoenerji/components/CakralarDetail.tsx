...
const SECTION_SHELL: Record<DetailSectionTone, { wrap: string; label: string }> = {
  ...
  fuchsia: { wrap: "rounded-lg border border-slate-200/70 bg-white p-4", label: "text-fuchsia-400" },
  indigo:  { wrap: "rounded-lg border border-slate-200/70 bg-white p-4", label: "text-indigo-400" },
  rose:    { wrap: "rounded-lg border border-slate-200/70 bg-white p-4", label: "text-rose-400" },
};
...
export default function CakralarDetail({ id }: { id: string }) {
  ...
  return (
    <div className="w-full min-w-0 max-w-none">
      ...
      {(infoSuccess || infoError || showRecordWithWarning) && (
        <div className="mb-4 flex flex-col gap-1.5 sm:flex-row">
          ...
          {showRecordWithWarning ? (
            <div className="flex-1 rounded-md border border-amber-200 bg-amber-50/80 px-3 py-1.5 text-[12px] font-medium text-amber-700">
              {errorMessage}
            </div>
          ) : null}
        </div>
      ...
      <div className="mb-5">
        {badge ? (
          <span className="mb-2 inline-flex items-center gap-1.5 rounded-full bg-fuchsia-50 px-2.5 py-0.5 text-[10px] font-medium text-fuchsia-700 ring-1 ring-fuchsia-200/60">
        ...
        <p className="mt-1.5 text-[11px] text-slate-400">{formatDate(record.created_at)}</p>
      </div>
