...
type Tab = "bu-ay" | "altin" | "diger" | "word";
...
export default function HacamatPage() {
  ...
  const activeMonthData = activeTab === "bu-ay" ? buAyData : digerData;
                ...
                return (
                  <div key={yr} className="rounded-[18px] border border-white/80 bg-white/70 p-3 shadow-sm backdrop-blur-md sm:p-4">
                    ...
                    {days.length === 0 ? (
                      ...
                      <div className="space-y-1.5">
                        {days.map((d, i) => (
                          <div key={i} className="flex items-center gap-3 rounded-xl border border-amber-200/60 bg-amber-50/80 px-3 py-2.5">
        ...
        {activeTab === "word" && (
          <section className="space-y-4">
            ...
            <div className="rounded-[18px] border border-white/80 bg-white/70 p-3 shadow-sm backdrop-blur-md sm:p-4">
              ...
              <p className="mb-3 text-[10px] text-slate-400">Kural ekleyebilir veya silebilirsiniz.</p>
              {(["oncesi", "sonrasi", "genel"] as RuleCategory[]).map(cat => {
                const catRules = rules.filter(r => r.category === cat);
                if (!catRules.length) return null;
                const catLabel = cat === "oncesi" ? "Hacamat Öncesi" : cat === "sonrasi" ? "Hacamat Sonrası" : "Genel Kurallar";
                return (
                  <div key={cat} className="mb-3">
                    <p className="mb-1.5 text-[8px] font-black uppercase tracking-[0.15em] text-slate-400">{catLabel}</p>
              ...
              <div className="mt-3 border-t border-slate-100 pt-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  <select value={newRuleCat} onChange={e => setNewRuleCat(e.target.value as RuleCategory)}
                  ...
                  <input id={newRuleId} type="text" placeholder="Kural metni girin…"
            ...
            {/* Uzman Notları */}
            <div className="rounded-[18px] border border-white/80 bg-white/70 p-3 shadow-sm backdrop-blur-md sm:p-4">
              <p className="mb-1 text-[9px] font-black uppercase tracking-[0.2em] text-teal-700">📝 Hacamat Uzmanı Notları</p>
            ...
            <div className="flex justify-end">
              <button onClick={handleWordReport} disabled={isGenerating}
                ...
                {isGenerating ? "Hazırlanıyor…" : `${MONTH_NAMES_TR[wordMonth]} ${wordYear} — Word Raporu İndir`}
              </button>
            </div>
          </section>
        )}
