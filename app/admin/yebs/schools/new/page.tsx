...
// ============================================================
// YEBS A8 — Yeni ekol (school) oluşturma formu
// tradition_id zorunlu (TraditionPicker ile seçilir); native üçlü opsiyonel.
...
export default function NewSchoolPage() {
  ...
  const canSubmit = traditionId !== null && slug.trim() !== "" && nameTr.trim() !== "" && !saving;

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!canSubmit || !traditionId) return;
    setSaving(true);
    ...
    const r = await schoolsApi.create(body);
    setSaving(false);

    if (r.ok) {
      showToast({ type: "success", title: "Oluşturuldu", message: "Ekol oluşturuldu." });
  ...
  return (
    <YebsPageShell>
      ...
      <h1 className="mb-4 text-lg font-black text-slate-900">Yeni Ekol</h1>
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl rounded-2xl border border-slate-200 bg-white/70 p-4 sm:p-5">
        <div className="mb-4">
          <TraditionPicker
            ...
            onPick={(pid, label) => { setTraditionId(pid); setTraditionLabel(label); }}
          />
          <p className="mt-1 text-[11px] text-slate-400">Ekolün bağlanacağı üst gelenek (zorunlu).</p>
        </div>

        <div className="grid gap-3 sm:grid-cols-2">
          <Field label="Kısa ad (slug) *" hint="Gelenek içinde benzersiz teknik kimlik.">
            <TextInput value={slug} onChange={(e) => setSlug(e.target.value)} required placeholder="ornek-ekol" />
          </Field>
          <Field label="Ad (Türkçe) *">
            <TextInput value={nameTr} onChange={(e) => setNameTr(e.target.value)} required placeholder="Ekol adı" />
          </Field>
        </div>
