...
// ============================================================
// YEBS A8 — Ekol (school) detay: sekmeler + düzenleme + yaşam döngüsü
// - Genel: canonical alan düzenleme (yalnız taslakta), reason zorunlu (ReasonPrompt)
// - Bağlantılar: üst gelenek bilgisi + bağlantı
...
import { schoolsApi, traditionsApi } from "@/app/admin/yebs/adminYebsApi";
...
const COLL = "schools";
const GROUP = ENTITY_LIFECYCLE_GROUP.school;
...
function toDraft(row: SchoolRow): Draft {
  return {
    slug: row.slug,
    name_tr: row.name_tr,
...
export default function SchoolDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [traditionName, setTraditionName] = useState<string | null>(null);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    setError(null);
    const r = await schoolsApi.detail(id);
    if (r.ok) {
      setRow(r.data);
      setDraft(toDraft(r.data));
      // Üst gelenek adını göstermek için hafif ek çağrı (başarısızlık bloklamaz).
      void traditionsApi.detail(r.data.tradition_id).then((tr) => {
        if (tr.ok) setTraditionName(tr.data.name_tr);
      });
    } else {
  ...
  const linksTab = (
    <div className="max-w-2xl rounded-2xl border border-slate-200 bg-white/70 p-4">
      <p className="text-xs font-bold uppercase tracking-wide text-slate-400">Üst gelenek</p>
      <Link
        href={`/admin/yebs/traditions/${row.tradition_id}`}
        className="mt-1 inline-flex items-center gap-1.5 text-sm font-semibold text-violet-700 hover:underline"
      >
        {traditionName ?? row.tradition_id} →
      </Link>
      <p className="mt-1 font-mono text-[11px] text-slate-400">{row.tradition_id}</p>
      <p className="mt-3 text-xs text-slate-500">Bir ekolün bağlı olduğu gelenek oluşturmadan sonra değiştirilemez.</p>
    </div>
  );
    ...
    <YebsPageShell>
      ...
      {showReason && (
        <ReasonPrompt
          ...
          submit={async (reason) => {
            setSaving(true);
            const r = await schoolsApi.update(id, { ...buildPatch(), expected_updated_at: row.updated_at, reason });
            setSaving(false);
            if (r.ok) showToast({ type: "success", title: "Kaydedildi", message: "Ekol güncellendi." });
            return r;
          }}
      ...
      {pendingAction && (
        <LifecycleModal
          ...
          fetchEligibility={(target) => schoolsApi.eligibility(id, target).then((r) => (r.ok ? r.data : null))}
          submit={(reason) => schoolsApi.transition(id, { target_status: pendingAction.target, expected_updated_at: row.updated_at, reason })}
