...
export default function DigerUrunStokPage() {
  ...
  function commitSale() {
    ...
    try {
      ...
      if (!isDemo && activeTenantId) {
        void syncOtherInventoryToDb(activeTenantId, updated).then(({ error }) => {
          if (error) console.warn("[other] Supabase sync hatası (satış):", error);
        });
      }
    } finally {
  ...
  async function deleteSelectedSales() {
    ...
    if (!isDemo && activeTenantId) {
      void syncOtherInventoryToDb(activeTenantId, inv).then(({ error }) => {
        if (error) console.warn("[other] Supabase sync hatası (stok iadesi):", error);
      });
    }
