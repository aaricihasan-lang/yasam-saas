...
import {
  deleteOtherInventoryItems,
  loadOtherInventoryForTenant,
  upsertOtherInventoryItem,
...
/** Canonical DB kategori satışları → OtherSaleRecord (snapshot; iptal için saleId). */
function dbToOtherRecords(dbSales: DbCategorySale[]): OtherSaleRecord[] {
  return dbSales.map((s) => {
    const lines: OtherSaleLine[] = s.items.map((it) => ({
      ...
      saleUnit: (it.unit || "adet") as OtInputUnit,
      saleBaseQty: dbNum(it.quantity),
...
export default function DigerUrunStokPage() {
  ...
  const reloadInv = useCallback(async () => {
    ...
    if (demo) {
      ...
      setInventory(loadOtherInventory());
      return;
    }
    const tenantId = await getSyncedTenantId();
    setActiveTenantId(tenantId);
    const { items } = await loadOtherInventoryForTenant(tenantId);
    setInventory(items);
  }, []);
  // USM: kategori "Satış Geçmişi" canonical DB'den (merkezî ile aynı veri). Demo localStorage.
  const reloadSales = useCallback(async () => {
    const demo = readYasamUser()?.is_demo_account === true;
    if (demo) { setSales(loadOtherSales()); return; }
    const { sales: dbSales } = await loadDbCategorySales("other");
    setSales(dbToOtherRecords(dbSales));
  }, []);
  ...
  async function handleSaveStock() {
    ...
    if (!isDemo && activeTenantId) {
      ...
      if (target) {
        const res = await upsertOtherInventoryItem(activeTenantId, target);
        if (!res.ok) {
          setMsg(
  ...
  async function deleteSelected() {
    ...
    if (!isDemo && activeTenantId && removed.length > 0) {
      const res = await deleteOtherInventoryItems(activeTenantId, removed);
      await reloadInv();
      if (!res.ok) {
  ...
  async function commitSale() {
    ...
    try {
      ...
      const lines = basket.flatMap((r) =>
        r.lines.map((l) => ({ clientId: l.productId, quantity: l.saleBaseQty, markupPct: r.profit_pct })),
      );
      const res = await createCategorySale({ category: "other", idempotencyKey: newIdempotencyKey(), lines });
      if (!res.ok) {
