...
const panelClass =
  "w-full rounded-[28px] border-2 border-lime-200/80 bg-white/85 p-6 shadow-[0_18px_50px_rgba(15,23,42,0.08)] backdrop-blur-xl sm:p-8";

const inputClass =
  "h-14 w-full rounded-2xl border-2 border-lime-200 bg-white px-4 text-base font-semibold text-slate-900 outline-none transition focus:border-lime-500 focus:ring-4 focus:ring-lime-200/50";

const btnPrimary =
  "inline-flex h-14 items-center justify-center rounded-2xl border-2 border-lime-400 bg-gradient-to-r from-lime-100 to-yellow-100 px-8 text-base font-black text-lime-900 shadow-md transition hover:scale-[1.02]";

const btnSecondary =
  "inline-flex h-12 items-center justify-center rounded-2xl border-2 border-lime-200 bg-lime-50 px-6 text-sm font-black text-slate-800 transition hover:bg-lime-100";
        ...
        <div className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-[28px] border-2 bg-white shadow-2xl">
          ...
          <div className="overflow-auto p-6">
            <table className="w-full text-base">
              <thead>
                <tr className="text-left text-sm font-black text-lime-800">
    ...
    <main className={pageBg}>
      ...
      <div className={pageShell}>
        ...
        <header className={`${panelClass} mb-8`}>
          <p className="text-sm font-black uppercase tracking-[0.3em] text-lime-700">Diğer Ürünler</p>
          <h1 className="mt-3 text-4xl font-black xl:text-5xl">Diğer Ürünler</h1>
          <p className="mt-4 text-lg text-slate-600">
            Kategoriye sığmayan tüm ürünler — esnek grup, varyasyon ve ölçü — adet, gram/kg ve ml/litre — birim maliyet otomatik; birim maliyet satış miktarına göre hesaplanır.
          </p>
        </header>
          ...
          <div className="w-full space-y-6">
            <section className={panelClass}>
              ...
              <p className="mb-6 text-sm font-semibold text-slate-600">
                Beden, renk, model gibi varyasyonlar için ayrı stok satırı açabilirsiniz.
              </p>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
                ...
                <label className="block">
                  <span className="mb-2 block text-sm font-black">Alt kategori / ürün tipi</span>
                ...
                <label className="block">
                  <span className="mb-2 block text-sm font-black">Varyasyon türü</span>
                ...
                <label className="block">
                  <span className="mb-2 block text-sm font-black">Varyasyon değeri</span>
                  <input
                    className={inputClass}
                    placeholder="M, Kırmızı, Set-A…"
              ...
              {stockUnitPreview ? (
                <div className="mt-4 rounded-2xl border-2 border-lime-200/90 bg-gradient-to-r from-lime-50/90 to-yellow-50/80 p-5">
                  <p className="text-sm font-black uppercase tracking-wide text-lime-800">Birim maliyet özeti</p>
                  ...
                  <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {stockUnitPreview.costPer != null ? (
                      <div className="rounded-xl border border-lime-200 bg-white/90 px-4 py-3">
                    ...
                    {stockUnitPreview.salePer != null ? (
                      <div className="rounded-xl border border-yellow-200 bg-white/90 px-4 py-3">
                        <p className="text-xs font-black text-yellow-700">Birim satış</p>
                        <p className="mt-1 text-lg font-black text-slate-900">
              ...
              <div className="mt-6 flex flex-wrap items-center gap-4">
                ...
                <p className="ml-auto font-black text-lime-900">Stok değeri: {fmtMoney(stockValue)}</p>
              </div>
            </section>
            ...
            <section className={panelClass}>
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[1320px] text-base">
                  <thead>
                    <tr className="text-sm font-black uppercase text-lime-800">
                  ...
                  <tbody>
                    {displayed.map((it, idx) => (
                      <tr key={it.id} className="border-t bg-white/80">
                        ...
                        <td className="p-3">{it.subCategory || "—"}</td>
                        ...
                        <td className="p-3">{it.variationDetail ? `${it.variationDetail} (${it.variationKind})` : it.variationKind}</td>
        ...
        {tab === "pricing" && (
          <div className="grid w-full gap-6 xl:grid-cols-[1.55fr_1fr]">
            <section className={`${panelClass} space-y-6`}>
              ...
              {picked ? (
                <div className="rounded-2xl border border-lime-200 bg-lime-50/80 px-5 py-4">
                  <p className="text-sm font-black text-lime-800">Kayıtlı birim maliyet</p>
                  <p className="mt-1 text-lg font-black text-slate-900">
              ...
              {previewLine && !("error" in previewLine) && picked ? (
                <div className="space-y-3">
                  <div className="grid gap-4 sm:grid-cols-3">
                    <div className="rounded-2xl border border-lime-200 bg-lime-50 p-4 text-center font-black">
                      Maliyet: {fmtMoney(previewLine.lineCost)}
                    </div>
                    <div className="rounded-2xl border border-yellow-200 bg-yellow-50 p-4 text-center font-black">
                      Satış:{" "}
                      {fmtMoney(
                  ...
                  <p className="rounded-xl border border-lime-100 bg-white/90 px-4 py-3 text-center text-sm font-bold text-slate-800">
                    {formatLineCostBreakdown(
            ...
            <section className={panelClass}>
              ...
              <div className="space-y-3">
                {basket.map((rec, i) => (
                  <div key={i} className="rounded-2xl border bg-lime-50/50 p-4">
