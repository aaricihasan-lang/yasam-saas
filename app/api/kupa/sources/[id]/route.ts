...
/** /api/kupa/sources/[id] — tek kaynak oku/güncelle/sil. */
...
export async function GET(
  ...
  if (!id) return NextResponse.json({ ok: false, error: "Kaynak id gerekli." }, { status: 400 });
  const res = await getEntity(guard.db, CUPPING_TABLES.sources, guard.tenantId, id);
...
export async function PATCH(
  ...
  const res = await updateEntity(db, CUPPING_TABLES.sources, tenantId, id, fields);
  if (!res.ok) return res.response;
  return NextResponse.json({ ok: true, source: res.data });
}
...
export async function DELETE(
  ...
  const res = await deleteEntity(db, CUPPING_TABLES.sources, tenantId, id);
  if (!res.ok) return res.response;
