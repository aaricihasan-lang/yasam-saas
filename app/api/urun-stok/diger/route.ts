...
/**
 * /api/urun-stok/diger — other_inventory (Diğer Ürünler ürün-stok) güvenli
 * server kapısı (K-2). GET (liste), POST (ekle), PATCH (güncelle, body.id),
 * DELETE (?id veya body.id). tenant_id daima oturumdan; her işlem
 * .eq("tenant_id", tenantId). oil/soap/accessory route deseni ile birebir aynı.
 */
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  const { data, error } = await db.from("other_inventory").insert(payload).select("id").single();
  if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
...
export async function PATCH(req: NextRequest): Promise<Response> {
  ...
  const { data, error } = await db
    .from("other_inventory").update(fields)
