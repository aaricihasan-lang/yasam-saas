...
import { CONCEPT_RELATION_STATUS_VALUES } from "@/lib/yebs/service/conceptRelationTransitions";

export const runtime = "nodejs";

type RouteContext = { params: Promise<{ id: string }> };

/** GET /api/admin/yebs/relations/[id]/eligibility?target_status=... (A7 read-only). */
export async function GET(req: NextRequest, ctx: RouteContext): Promise<Response> {
  ...
  if (!(CONCEPT_RELATION_STATUS_VALUES as readonly string[]).includes(targetStatus)) {
  ...
  const res = await a7Eligibility(db, "concept_relation", adminId, id, targetStatus);
