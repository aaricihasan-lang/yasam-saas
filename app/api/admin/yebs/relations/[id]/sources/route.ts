...
import {
  ...
  YEBS_CONCEPT_RELATION_SOURCE_VERIFICATION_STATUSES,
  type YebsConceptRelationSourceEvidenceLayer,
  type YebsConceptRelationSourceRole,
  type YebsConceptRelationSourceRationaleStatus,
  type YebsConceptRelationSourceVerificationStatus,
} from "@/lib/yebs/service/conceptRelationSources";
import {
  attachConceptRelationSource,
  type AttachConceptRelationSourceInput,
  type AttachConceptRelationSourceErrorCode,
 ...
 * editoryal. DELETE burada değil (detail).
 */
...
// A5B metin sınırları (D9 sözleşmesi).
const LIMITS = {
  locator_text: 2000,
  url_fragment: 2000,
...
export async function GET(req: NextRequest, ctx: RouteContext): Promise<Response> {
  ...
  // evidence_layer
  let evidenceLayer: YebsConceptRelationSourceEvidenceLayer | undefined;
  const rawEvidence = sp.get("evidence_layer");
  if (rawEvidence !== null && rawEvidence !== "") {
    if (!(YEBS_CONCEPT_RELATION_SOURCE_EVIDENCE_LAYERS as readonly string[]).includes(rawEvidence)) {
      return bad("Geçersiz evidence_layer değeri.", "YEBS_INVALID_EVIDENCE_LAYER");
    }
    evidenceLayer = rawEvidence as YebsConceptRelationSourceEvidenceLayer;
  }
  // source_role
  let sourceRole: YebsConceptRelationSourceRole | undefined;
  const rawRole = sp.get("source_role");
  if (rawRole !== null && rawRole !== "") {
    if (!(YEBS_CONCEPT_RELATION_SOURCE_ROLES as readonly string[]).includes(rawRole)) {
      return bad("Geçersiz source_role değeri.", "YEBS_INVALID_SOURCE_ROLE");
    }
    sourceRole = rawRole as YebsConceptRelationSourceRole;
  }
  // rationale_status
  let rationaleStatus: YebsConceptRelationSourceRationaleStatus | undefined;
  const rawRat = sp.get("rationale_status");
  if (rawRat !== null && rawRat !== "") {
    if (!(YEBS_CONCEPT_RELATION_SOURCE_RATIONALE_STATUSES as readonly string[]).includes(rawRat)) {
      return bad("Geçersiz rationale_status değeri.", "YEBS_INVALID_RATIONALE_STATUS");
    }
    rationaleStatus = rawRat as YebsConceptRelationSourceRationaleStatus;
  }
  // verification_status
  let verificationStatus: YebsConceptRelationSourceVerificationStatus | undefined;
  const rawVer = sp.get("verification_status");
  if (rawVer !== null && rawVer !== "") {
    if (!(YEBS_CONCEPT_RELATION_SOURCE_VERIFICATION_STATUSES as readonly string[]).includes(rawVer)) {
      return bad("Geçersiz verification_status değeri.", "YEBS_INVALID_VERIFICATION_STATUS");
    }
    verificationStatus = rawVer as YebsConceptRelationSourceVerificationStatus;
  }
  ...
  const result = await listConceptRelationSources(db, relationId, {
    limit,
    offset,
    sourceId,
    evidenceLayer,
  ...
  if (!result.ok) {
    if (result.code === "YEBS_RELATION_SOURCE_RELATION_NOT_FOUND") {
      return NextResponse.json({ ok: false, error: "İlişki kaydı bulunamadı.", code: result.code }, { status: 404 });
    }
    return NextResponse.json(
      { ok: false, error: "YEBS ilişki kaynak bağları alınamadı.", code: result.code },
      { status: 500 },
    );
  }
...
const ALLOWED_BODY_KEYS = [
  "source_id", "evidence_layer", "source_role", "locator_text", "url_fragment",
  "source_original_excerpt", "source_original_language_tag", "source_original_script_code",
  "transliteration", "transliteration_scheme", "faithful_translation",
  "translation_language_tag", "rationale", "rationale_status", "reason",
] as const;
...
function mapAttachError(code: AttachConceptRelationSourceErrorCode): Response {
  switch (code) {
    ...
    case "YEBS_RELATION_SOURCE_ATTACH_FAILED":
    default:
      return NextResponse.json({ ok: false, error: "Kaynak bağı oluşturulamadı.", code: "YEBS_RELATION_SOURCE_ATTACH_FAILED" }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest, ctx: RouteContext): Promise<Response> {
  ...
  // required strings (nonblank; coercion YOK). evidence_layer zorunlu.
  for (const key of ["source_id", "evidence_layer", "source_role", "rationale_status"] as const) {
    const v = obj[key];
  ...
  if (!(YEBS_CONCEPT_RELATION_SOURCE_EVIDENCE_LAYERS as readonly string[]).includes(evidenceLayer)) return invalidBody();
  const sourceRole = obj.source_role as string;
  if (!(YEBS_CONCEPT_RELATION_SOURCE_ROLES as readonly string[]).includes(sourceRole)) return invalidBody();
  const rationaleStatus = obj.rationale_status as string;
  if (!(YEBS_CONCEPT_RELATION_SOURCE_RATIONALE_STATUSES as readonly string[]).includes(rationaleStatus)) return invalidBody();
  ...
  const input: AttachConceptRelationSourceInput = {
    sourceId,
    evidenceLayer,
    sourceRole,
    rationaleStatus,
  ...
  const result = await attachConceptRelationSource(db, adminId, relationId, input);
