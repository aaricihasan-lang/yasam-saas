...
import { CONCEPT_STATUS_VALUES } from "@/lib/yebs/service/conceptTransitions";

export const runtime = "nodejs";

type RouteContext = { params: Promise<{ id: string }> };

/** GET /api/admin/yebs/concepts/[id]/eligibility?target_status=... (A7 read-only). */
export async function GET(req: NextRequest, ctx: RouteContext): Promise<Response> {
  ...
  if (!(CONCEPT_STATUS_VALUES as readonly string[]).includes(targetStatus)) {
  ...
  const res = await a7Eligibility(db, "concept", adminId, id, targetStatus);
