...
import { SCHOOL_STATUS_VALUES } from "@/lib/yebs/service/schoolTransitions";

export const runtime = "nodejs";

type RouteContext = { params: Promise<{ id: string }> };

/** GET /api/admin/yebs/schools/[id]/eligibility?target_status=... (A7 read-only). */
export async function GET(req: NextRequest, ctx: RouteContext): Promise<Response> {
  ...
  if (!(SCHOOL_STATUS_VALUES as readonly string[]).includes(targetStatus)) {
  ...
  const res = await a7Eligibility(db, "school", adminId, id, targetStatus);
