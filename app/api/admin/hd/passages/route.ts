...
import { createPassage, deletePassage, listPassages, updatePassage } from "@/lib/human-design/admin/centralContentPersistence";
...
/** /api/admin/hd/passages — kaynak pasajı CRUD (source_specific_note dahil). */
export async function GET(req: NextRequest): Promise<Response> {
  const guard = await verifyAdminRequest(req);
  if (!guard.ok) return guard.response;
  const sourceId = req.nextUrl.searchParams.get("sourceId")?.trim();
  if (!sourceId) return NextResponse.json({ ok: false, error: "sourceId gerekli." }, { status: 400, headers: NO_STORE });
  const r = await listPassages(guard.db, sourceId);
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  try {
    const r = await createPassage(guard.db, guard.adminId, raw);
    if (!r.ok) return NextResponse.json({ ok: false, error: r.error.message }, { status: st(r.error), headers: NO_STORE });
...
export async function PATCH(req: NextRequest): Promise<Response> {
  ...
  try {
    const r = await updatePassage(guard.db, guard.adminId, id, raw);
    if (!r.ok) return NextResponse.json({ ok: false, error: r.error.message }, { status: st(r.error), headers: NO_STORE });
    return NextResponse.json({ ok: true, id: r.data.id }, { headers: NO_STORE });
  } catch (e) { return auditCatch(e); }
}
...
export async function DELETE(req: NextRequest): Promise<Response> {
  ...
  if (!id) return NextResponse.json({ ok: false, error: "id gerekli." }, { status: 400, headers: NO_STORE });
  try {
    const r = await deletePassage(guard.db, guard.adminId, id);
    if (!r.ok) return NextResponse.json({ ok: false, error: r.error.message }, { status: st(r.error), headers: NO_STORE });
    return NextResponse.json({ ok: true, id: r.data.id }, { headers: NO_STORE });
  } catch (e) { return auditCatch(e); }
}
