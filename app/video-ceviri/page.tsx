...
import { ArrowLeft, Clock, FileText, Loader2, Video } from "lucide-react";
            ...
            {STEPS.map((step, i) => (
              <div
                ...
                className={`flex flex-col items-center gap-3 px-5 py-6 text-center ${
                  i < STEPS.length - 1 ? "sm:border-r sm:border-white/10" : ""
                }`}
              >
            ...
            {jobsLoading ? (
              ...
              <div className="space-y-3">
                {jobs.map((job) => (
                  <div
                    key={job.id}
                    className="flex items-center justify-between gap-4 rounded-2xl border border-slate-100/90 bg-slate-50/70 px-5 py-4"
                  >
                    <div className="flex min-w-0 items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs font-medium text-slate-400">
                          {formatFileSizeTr(job.file_size_bytes)} ·{" "}
                          {formatJobDateTr(job.created_at)}
                        </p>
                      </div>
                    </div>
                    <span
                      className={`inline-flex shrink-0 rounded-full px-3 py-1 text-xs font-bold ring-1 ${
