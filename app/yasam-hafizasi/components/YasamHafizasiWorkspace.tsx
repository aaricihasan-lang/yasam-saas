...
export function YasamHafizasiWorkspace() {
  ...
  const runSearch = useCallback(async (query: string, shared: boolean) => {
    ...
    setStatus("loading");
    const res = await fetchYhSearch({ q: trimmed, allowShared: shared }, ctrl.signal);
  ...
  const onAllowSharedChange = useCallback(
    (v: boolean) => {
      setAllowShared(v);
      if (submitted) void runSearch(submitted, v);
    },
    [submitted, runSearch],
  );
      ...
      >
        <div className="relative flex-1">
          ...
          <input
            ...
            aria-label="Yaşam Hafızası araması"
      ...
      {status === "error" ? (
        <WorkspaceEmpty
          ...
          action={
            <button
              type="button"
              onClick={() => void runSearch(submitted, allowShared)}
        ...
        response.results.length === 0 ? (
          ...
          <>
            <FilterBar
              ...
              onAllowSharedChange={onAllowSharedChange}
            />
