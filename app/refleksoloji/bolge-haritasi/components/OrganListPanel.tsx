...
export function OrganListPanel({ selectedOrgan, setSelectedOrgan }: OrganListPanelProps) {
  ...
  return (
    <aside className="flex min-h-0 flex-col rounded-[24px] border border-white/90 bg-white/80 p-4 shadow-[0_16px_40px_-18px_rgba(91,33,182,0.2)] ring-1 ring-violet-100/70 backdrop-blur-md sm:p-5">
      <h2 className="text-xs font-black uppercase tracking-[0.22em] text-violet-800/90">Organlar</h2>
      <label className="mt-3 block">
        ...
        <input
          ...
          className="w-full rounded-xl border border-violet-200/80 bg-violet-50/40 px-3.5 py-2.5 text-sm font-medium text-slate-800 placeholder:text-slate-400 outline-none ring-violet-300/40 transition focus:border-violet-300 focus:bg-white focus:ring-2"
        />
      </label>
            ...
            return (
              <li key={organ}>
                <button
                  ...
                  className={`flex w-full items-center gap-2.5 rounded-xl border px-3 py-2.5 text-left text-sm font-semibold transition-all duration-200 ${
                    isSelected
