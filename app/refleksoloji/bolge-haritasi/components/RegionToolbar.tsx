...
function toggleButtonClass(active: boolean, variant: "foot" | "view") {
  if (variant === "foot") {
    return `rounded-xl border px-3.5 py-2 text-xs font-bold transition-all duration-200 sm:px-4 sm:text-sm ${
      active
...
export function RegionToolbar({
  ...
  return (
    <div
      className="rounded-[22px] border border-white/90 bg-white/85 p-3 shadow-[0_12px_36px_-16px_rgba(91,33,182,0.2)] ring-1 ring-violet-100/70 backdrop-blur-md sm:p-4"
      role="toolbar"
    ...
    >
      <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-2.5">
        ...
        <button
          type="button"
          disabled
          className="cursor-default rounded-xl border border-violet-200/80 bg-violet-50/55 px-3.5 py-2 text-xs font-bold text-violet-900/90 opacity-90 transition sm:px-4 sm:text-sm"
        ...
        <span className="mx-1 hidden h-6 w-px bg-violet-200/80 sm:inline" aria-hidden />
