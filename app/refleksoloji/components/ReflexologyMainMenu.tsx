...
function ReflexologyHubCard({ module }: { module: HubModule }) {
  return (
    <Link
      href={module.href}
      className={`group relative flex h-full min-h-[138px] flex-col items-center justify-between overflow-hidden rounded-2xl border border-white/80 bg-white/70 p-4 text-center shadow-[0_14px_40px_-16px_rgba(91,33,182,0.28)] ring-1 backdrop-blur-md transition-all duration-300 hover:-translate-y-1.5 hover:bg-white/85 hover:shadow-[0_22px_50px_-14px_rgba(91,33,182,0.38)] active:translate-y-0 ${module.ring} ${module.hoverRing}`}
    >
      <div
      ...
      <div className="relative flex min-h-0 flex-1 flex-col items-center justify-center px-1 py-2">
...
function ReflexologyMainMenu() {
  return (
    <main className="relative h-screen overflow-hidden text-slate-900 antialiased">
      ...
      <div className="relative z-10 mx-auto flex h-full w-full max-w-[1180px] flex-col px-4 py-3 sm:px-6 lg:px-8 lg:py-4">
        <div className="flex shrink-0 items-start justify-between gap-4">
          <header className="min-w-0">
            <p className="text-[10px] font-black uppercase tracking-[0.34em] text-violet-700/90 sm:text-[11px]">
              Yaşam Sistemi
            </p>
            <h1 className="mt-1 bg-gradient-to-r from-violet-950 via-indigo-900 to-violet-800 bg-clip-text text-3xl font-black tracking-tight text-transparent sm:text-4xl lg:text-5xl">
              Refleksoloji
            </h1>
            <p className="mt-2 max-w-2xl text-base font-medium leading-relaxed text-slate-600/95 sm:text-lg">
              Ayak refleksoloji atlası, protokoller ve klinik çalışma alanı
            </p>
            <p className="mt-2.5 inline-flex max-w-full flex-wrap items-center gap-x-1.5 rounded-full border border-violet-200/70 bg-white/55 px-3 py-1.5 text-[10px] font-bold leading-snug text-violet-900/90 shadow-sm ring-1 ring-white/60 backdrop-blur-sm sm:text-[11px]">
        ...
        <section
          className="mt-3 flex min-h-0 flex-1 flex-col overflow-hidden rounded-[26px] border border-white/70 bg-white/40 shadow-[0_20px_56px_-22px_rgba(91,33,182,0.3)] ring-1 ring-white/50 backdrop-blur-xl sm:mt-3.5"
          style={{ height: "calc(100vh - 118px)" }}
        >
          <div className="flex min-h-0 flex-1 flex-col overflow-y-auto p-3.5 sm:p-4 lg:overflow-hidden lg:p-5">
            <div className="grid min-h-0 flex-1 grid-cols-1 gap-3 lg:grid-cols-6 lg:grid-rows-2 lg:gap-3.5 lg:overflow-hidden">
              {MENU_MODULES.map((module, index) => (
                <div key={module.href} className={`min-h-0 ${gridCellClass(index)}`}>
