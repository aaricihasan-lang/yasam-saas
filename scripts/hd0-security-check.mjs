...
check("eski dosya YALNIZ DB update sonrası ve tenant/client prefix ile temizlenir", /previousPath\.startsWith\(`\$\{guard\.tenantId\}\/\$\{clientId\}\/`\)/.test(upload));
...
check("silme yalnız tenant/client prefix'i ile", /currentPath\.startsWith\(`\$\{guard\.tenantId\}\/\$\{clientId\}\/`\)/.test(del));
...
check("path DB'den okunur + tenant/client prefix re-check", /path\.startsWith\(`\$\{guard\.tenantId\}\/\$\{clientId\}\/`\)/.test(signed));
...
check("görsel signed URL ile gösterilir (chart-image-url route)", /\/api\/hd\/chart-image-url/.test(comp));
check("DB path doğrudan img src DEĞİL (displayUrl state)", /src=\{displayUrl\}/.test(comp) && !/src=\{currentImageUrl\}/.test(comp));
check("legacy public URL geriye-uyumlu (otomatik dönüştürme yok)", /https\?:\\\/\\\//.test(comp));
