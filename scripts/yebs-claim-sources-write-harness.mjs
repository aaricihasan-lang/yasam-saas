// ============================================================
// YEBS API-A4B — Claim Source MUTATION route/service sözleşme + değişmezlik harness'i.
...
const REL = {
  ...
  mut: "lib/yebs/service/claimSourceMutations.ts",
};
...
if (Object.keys(S).length === 3) {
  ...
  check("attach allowlist 14 anahtar", (S.listRoute.match(/ALLOWED_BODY_KEYS = \[([\s\S]*?)\]/)?.[1].match(/"[a-z_]+"/g) || []).length === 14);
  check("attach required 3 (source_id/source_role/rationale_status)", /for \(const key of \["source_id", "source_role", "rationale_status"\]/.test(S.listRoute));
  {
    const m = S.listRoute.match(/ALLOWED_BODY_KEYS = \[([^\]]*)\]/);
    check("attach allowlist verification_status/claim_id/id/status İÇERMİYOR", m && !/"verification_status"|"claim_id"|"\bid\b"|"status"|"created_at"|"updated_at"/.test(m[1]));
  }
  check("update allowlist 14 (expected+reason+12)", (S.detailRoute.match(/PATCH_ALLOWED_KEYS = \[([\s\S]*?)\]/)?.[1].match(/"[a-z_]+"/g) || []).length === 14);
  {
    const m = S.detailRoute.match(/PATCH_ALLOWED_KEYS = \[([\s\S]*?)\]/);
    check("update allowlist source_id/claim_id/verification_status/id/timestamps İÇERMİYOR", m && !/"source_id"|"claim_id"|"verification_status"|"\bid\b"|"created_at"|"updated_at"/.test(m[1]));
  }
  ...
  check("attach enum source_role/rationale_status", /YEBS_CLAIM_SOURCE_ROLES as readonly string\[\]\)\.includes\(sourceRole\)/.test(S.listRoute) && /YEBS_CLAIM_SOURCE_RATIONALE_STATUSES as readonly string\[\]\)\.includes\(rationaleStatus\)/.test(S.listRoute));
  ...
  check("attach rpc çağrısı", /\.rpc\("yebs_attach_claim_source_with_audit"/.test(S.mut));
  check("update rpc çağrısı", /\.rpc\("yebs_update_claim_source_with_audit"/.test(S.mut));
  check("remove rpc çağrısı", /\.rpc\("yebs_remove_claim_source_with_audit"/.test(S.mut));
  ...
  check("canonical row guard", /isCanonicalClaimSourceRow/.test(S.mut));
  check("attach RPC param p_claim_id/p_rationale_status", /p_claim_id: claimId/.test(S.mut) && /p_rationale_status: input\.rationaleStatus/.test(S.mut));
  check("update RPC param p_patch/p_claim_source_id", /p_patch: patch/.test(S.mut) && /p_claim_source_id: claimSourceId/.test(S.mut));
  check("remove RPC param p_claim_source_id/p_expected_updated_at", /p_claim_source_id: claimSourceId/.test(S.mut) && /p_expected_updated_at: expectedUpdatedAt/.test(S.mut));
  ...
  check("attach claim not found 404", /YEBS_CLAIM_SOURCE_CLAIM_NOT_FOUND[\s\S]*?status:\s*404/.test(S.listRoute));
  check("attach source not found 404", /YEBS_CLAIM_SOURCE_SOURCE_NOT_FOUND[\s\S]*?status:\s*404/.test(S.listRoute));
  check("attach claim locked 409", /YEBS_CLAIM_SOURCE_CLAIM_LOCKED[\s\S]*?status:\s*409/.test(S.listRoute));
  check("update stale/locked/no-changes 409", /YEBS_CLAIM_SOURCE_STALE_UPDATE[\s\S]*?status:\s*409/.test(S.detailRoute) && /YEBS_CLAIM_SOURCE_CLAIM_LOCKED[\s\S]*?status:\s*409/.test(S.detailRoute) && /YEBS_CLAIM_SOURCE_NO_CHANGES[\s\S]*?status:\s*409/.test(S.detailRoute));
  check("detail not found 404", /YEBS_CLAIM_SOURCE_NOT_FOUND[\s\S]*?status:\s*404/.test(S.detailRoute));
  ...
  check("route'larda Claim/Source delete veya verification transition YOK", !/yebs_delete|verification_status:/i.test(S.detailRoute) && !/yebs_delete/i.test(S.listRoute));
}

console.log("\n[G] A4A + D6/D7 + AUD1 + A0/A1/A2/A3 + adminGuard + package git-blob DEĞİŞMEZLİĞİ");
const FROZEN = [
  // A4A commit dosyaları (bu turda DEĞİŞMEZ) — HEAD vs A4A commit karşılaştırması aşağıda ayrı
  "supabase/migrations/20260828000000_yebs_claim_mutations.sql",
  "app/api/admin/yebs/claims/route.ts",
  "app/api/admin/yebs/claims/[id]/route.ts",
  "lib/yebs/service/claims.ts",
  "lib/yebs/service/claimMutations.ts",
  // önceki fazlar (origin/main ile AYNI)
...
// A4A dosyaları: HEAD == A4A commit c82ae10 (bu tur onları değiştirmemeli)
const A4A_COMMIT = "c82ae104724fd8ce79adeb239dd38986b3ae3e5c";
const A4A_FILES = new Set([
  "supabase/migrations/20260828000000_yebs_claim_mutations.sql",
  "app/api/admin/yebs/claims/route.ts",
  "app/api/admin/yebs/claims/[id]/route.ts",
  "lib/yebs/service/claims.ts",
...
for (const rel of FROZEN) {
  ...
  const ref = A4A_FILES.has(rel) ? A4A_COMMIT : "origin/main";
...
if (!BASE_URL) skipped("canlı HTTP", "YEBS_HARNESS_BASE_URL yok");
  ...
  const tests = [
    ["POST attach 401", `${base}/api/admin/yebs/claims/${cid}/sources`, "POST"],
    ["PATCH claim-source 401", `${base}/api/admin/yebs/claims/${cid}/sources/${cid}`, "PATCH"],
    ["DELETE claim-source 401", `${base}/api/admin/yebs/claims/${cid}/sources/${cid}`, "DELETE"],
  ];
