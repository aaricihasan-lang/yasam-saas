 ...
 *   • tenantScopedSelect/Update/Delete HER ZAMAN `.eq("tenant_id", tenantId)` enjekte eder.
 *   • tenantInsertPayload tenant_id'yi enjekte eder ve client'ın gönderdiğini EZER (mass-assignment).
 *   • Primitive'ler tenant_id yoksa hiç sorgu kurmadan fail-closed olur.
 */
import {
  assertTenantId,
  tenantScopedSelect,
...
function main(): void {
  ...
  // ── B. Primitive'ler tenant filtresini HER ZAMAN enjekte eder ─────────────
  console.log("\nB. Primitive'ler tenant_id filtresini enjekte eder");
  const TID = "tenant-abc";
  {
    const { db, log } = recorder();
    tenantScopedSelect(db, "human_design_clients", TID, "*").eq("id", "x").maybeSingle();
    ok("select: op=select", log[0]!.op === "select");
    ok("select: tenant_id eq enjekte edildi", hasTenantEq(log[0]!, TID));
  }
  {
    const { db, log } = recorder();
    tenantScopedSelect(db, "human_design_reports", TID, "id", { count: "exact", head: true }).eq("client_id", "c1");
    ok("select(count): tenant_id eq enjekte edildi", hasTenantEq(log[0]!, TID));
  }
  {
    const { db, log } = recorder();
    tenantScopedUpdate(db, "human_design_charts", TID, { notes: "n" }).eq("id", "x").select("id");
    ok("update: op=update", log[0]!.op === "update");
    ok("update: tenant_id eq enjekte edildi", hasTenantEq(log[0]!, TID));
  }
  {
    const { db, log } = recorder();
    tenantScopedDelete(db, "human_design_reports", TID).eq("client_id", "c1");
    ok("delete: op=delete", log[0]!.op === "delete");
    ok("delete: tenant_id eq enjekte edildi", hasTenantEq(log[0]!, TID));
  }

  // ── C. Primitive'ler boş tenant'ta fail-closed (sorgu KURULMAZ) ───────────
  console.log("\nC. Boş tenant → primitive fail-closed");
  {
    const { db, log } = recorder();
    ok("tenantScopedSelect('') fırlatır", throws(() => tenantScopedSelect(db, "t", "", "*")));
    ok("boş tenant'ta sorgu kurulmadı (log boş)", log.length === 0);
  }
  ok("tenantScopedDelete('') fırlatır", throws(() => tenantScopedDelete(recorder().db, "t", "")));
  ok("tenantScopedUpdate('') fırlatır", throws(() => tenantScopedUpdate(recorder().db, "t", "", {})));
