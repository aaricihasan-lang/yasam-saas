/**
 * verify-diger-persistence.mjs — K-2 Diğer Ürünler DB kalıcılık canlı testi.
 * Yerel production (next start) + prod Supabase. Kayıtlar SİLİNMEZ.
 *
 *  FAZ A  login (esra, localhost) → storageState (tek login, paylaşılır)
 *  FAZ B  desktop: kayıt ekle → listede gör → birim maliyet/satış/stok doğrula
...
const NAME_DESK = `TEST DIGER K2 WEB ${TAG}`;
const NAME_MOB = `TEST DIGER K2 MOBIL ${TAG}`;
...
try {
  ...
  // ═══ FAZ B: DESKTOP EKLE ═══
  log("\n═══ FAZ B: Desktop diğer ürün kaydı ekle ═══");
  const dc = await browser.newContext({ viewport: { width: 1440, height: 900 }, storageState });
  const dp = await dc.newPage();
  await dp.goto(BASE + "/urun-stok/diger", { waitUntil: "domcontentloaded", timeout: 45000 });
  await dp.waitForTimeout(1500);
  // Adet ölçü, 8 adet, alış 400 → 50₺/adet; satış 800 → 100₺/adet
  await fillStockForm(dp, { name: NAME_DESK, measure: "Adet", unit: "adet", qty: 8, cost: 400, sale: 800, profit: 100 });
  ...
  rec("Stok gösterimi 8 adet", /\b8 adet\b/.test(rowText), rowText.replace(/\s+/g, " ").slice(0, 110));
  rec("Birim maliyet ₺50,00/adet (400/8)", /50,00\s*\/\s*adet/.test(rowText));
  ...
  const dbCount = await dp.evaluate(async (nm) => {
    ...
    const r = await fetch("/api/urun-stok/diger", { headers: { "x-user-id": uu.id, "x-session-token": tt }, cache: "no-store" });
  ...
  await fp.goto(BASE + "/urun-stok/diger", { waitUntil: "domcontentloaded", timeout: 45000 });
  ...
  await mp.goto(BASE + "/urun-stok/diger", { waitUntil: "domcontentloaded", timeout: 45000 });
  await mp.waitForTimeout(2000);
  // ML/Litre, 500 ml, alış 250 → 0,50₺/ml
  await fillStockForm(mp, { name: NAME_MOB, measure: "ML / Litre", unit: "ml", qty: 500, cost: 250, sale: 500, profit: 100 });
  ...
  await dp2.goto(BASE + "/urun-stok/diger", { waitUntil: "domcontentloaded", timeout: 45000 });
  ...
  rec("Mobil kayıt 500 ml + ₺0,50/ml (250/500)", /\b500 ml\b/.test(mobRow) && /0,50\s*\/\s*ml/.test(mobRow), mobRow.replace(/\s+/g, " ").slice(0, 110));
  await dc2.close();
