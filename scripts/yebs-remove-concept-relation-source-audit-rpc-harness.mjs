// ============================================================
// YEBS API-A5B — Concept Relation Source REMOVE (detach) audit RPC harness'i (statik SQL). FAIL → exit 1.
// ============================================================
...
if (sql) {
  const body = (sql.match(/CREATE FUNCTION public\.yebs_remove_concept_relation_source_with_audit\([\s\S]*?\$\$;/) || [""])[0];

  console.log("\n[A] REMOVE RPC imza + güvenlik");
  check("CREATE FUNCTION remove (OR REPLACE değil)", /CREATE FUNCTION public\.yebs_remove_concept_relation_source_with_audit\(/.test(sql) && !/CREATE OR REPLACE FUNCTION public\.yebs_remove_concept_relation_source_with_audit/.test(sql));
  check("RETURNS public.yebs_concept_relation_sources", /yebs_remove_concept_relation_source_with_audit[\s\S]*?RETURNS public\.yebs_concept_relation_sources/.test(sql));
  check("SECURITY DEFINER + güvenli search_path", /yebs_remove_concept_relation_source_with_audit[\s\S]*?SECURITY DEFINER[\s\S]*?SET search_path = pg_catalog, public/.test(sql));
  check("7 param (…relation_source_id, expected_updated_at, reason)", /p_concept_relation_id\s+uuid,\s*p_relation_source_id\s+uuid,\s*p_expected_updated_at timestamptz,\s*p_reason\s+text/.test(body));
  ...
  const iDelete = body.indexOf("DELETE FROM public.yebs_concept_relation_sources");
  check("önce audit, sonra DELETE (sıra)", iAudit > 0 && iDelete > 0 && iAudit < iDelete);
  check("audit action=remove entity=concept_relation_source", /'remove', 'concept_relation_source',/.test(body));
  check("full previous snapshot (previous=existing, new=NULL)", /'committed', to_jsonb\(v_existing\), NULL, ARRAY\[\]::text\[\]/.test(body));
  check("yalnız junction DELETE (WHERE id)", /DELETE FROM public\.yebs_concept_relation_sources WHERE id = p_relation_source_id;/.test(body));
  check("removed canonical row return", /RETURN v_existing;/.test(body));

  console.log("\n[D] Yasak: Relation/Source silme yok + EXECUTE + tam 3 RPC");
  ...
  check("remove RPC EXECUTE yalnız service_role", /GRANT EXECUTE ON FUNCTION public\.yebs_remove_concept_relation_source_with_audit\([\s\S]*?\) TO service_role/.test(sql));
  check("remove RPC service_role REVOKE", /REVOKE ALL ON FUNCTION public\.yebs_remove_concept_relation_source_with_audit\([\s\S]*?\) FROM service_role/.test(sql));
