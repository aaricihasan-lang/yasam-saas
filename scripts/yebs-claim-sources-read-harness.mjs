// ============================================================
// YEBS API-A4BR — Claim Sources SALT-OKUNUR API harness'i (statik sözleşme). FAIL → exit 1.
// ============================================================
...
if (list && detail && svc) {
  ...
  console.log("\n[B] Canonical 18 alan + fail-closed + explicit SELECT");
  check("YEBS_CLAIM_SOURCE_COLUMNS açık liste, select * yok",
    /YEBS_CLAIM_SOURCE_COLUMNS\s*=\s*\n?\s*"id, claim_id, source_id,[\s\S]*?created_at, updated_at"/.test(svc) && !/\.select\(\s*["'`]\*/.test(svc));
  ...
  check("fail-closed list (every isCanonicalClaimSourceRow)", /rows\.every\(isCanonicalClaimSourceRow\)/.test(svc));

  console.log("\n[C] Enum kümeleri exact");
  check("YEBS_CLAIM_SOURCE_ROLES 4 değer", (svc.match(/YEBS_CLAIM_SOURCE_ROLES = \[([\s\S]*?)\]/)?.[1].match(/"[a-z_]+"/g) || []).length === 4);
  check("role 4 exact", ["primary_support", "supporting", "contradiction", "context"].every(t => new RegExp(`"${t}"`).test(svc)));
  check("YEBS_CLAIM_SOURCE_RATIONALE_STATUSES 2 değer", (svc.match(/YEBS_CLAIM_SOURCE_RATIONALE_STATUSES = \[([\s\S]*?)\]/)?.[1].match(/"[a-z_]+"/g) || []).length === 2);
  check("rationale_status 2 exact", ["from_source", "source_gives_no_rationale"].every(t => new RegExp(`"${t}"`).test(svc)));
  check("YEBS_CLAIM_SOURCE_VERIFICATION_STATUSES 3 değer", (svc.match(/YEBS_CLAIM_SOURCE_VERIFICATION_STATUSES = \[([\s\S]*?)\]/)?.[1].match(/"[a-z_]+"/g) || []).length === 3);
  ...
  check("collection claim_id ile sınırlı", /\.eq\("claim_id",\s*claimId\)/.test(svc));
  check("parent Claim existence kontrolü (404)", /from\("yebs_claims"\)\.select\("id"\)\.eq\("id",\s*claimId\)/.test(svc) && /YEBS_CLAIM_SOURCE_CLAIM_NOT_FOUND/.test(svc));
  ...
  check("collection claimId strict UUID", /UUID_RE\.test\(claimId\)/.test(list));
  check("source_id strict UUID", /UUID_RE\.test\(rawSource\)/.test(list));
  check("source_role enum doğrulama", /YEBS_CLAIM_SOURCE_ROLES\s+as\s+readonly\s+string\[\]\)\.includes/.test(list));
  check("rationale_status enum doğrulama", /YEBS_CLAIM_SOURCE_RATIONALE_STATUSES\s+as\s+readonly\s+string\[\]\)\.includes/.test(list));
  check("verification_status enum doğrulama", /YEBS_CLAIM_SOURCE_VERIFICATION_STATUSES\s+as\s+readonly\s+string\[\]\)\.includes/.test(list));
  ...
  check("detail her iki UUID strict", /UUID_RE\.test\(claimId\) \|\| !UUID_RE\.test\(claimSourceId\)/.test(detail));
  check("detail path aidiyeti (service claim_id eşleşmesi)", /data\.claim_id !== claimId/.test(svc));
  check("detail 404 map", /YEBS_CLAIM_SOURCE_NOT_FOUND[\s\S]*?status:\s*404/.test(detail));
  check("collection parent 404 map", /YEBS_CLAIM_SOURCE_CLAIM_NOT_FOUND[\s\S]*?status:\s*404/.test(list));
}
...
if (!BASE_URL) skipped("canlı HTTP", "YEBS_HARNESS_BASE_URL yok");
  ...
  for (const [n, u] of [["list GET 401", `${base}/api/admin/yebs/claims/${cid}/sources`], ["detail GET 401", `${base}/api/admin/yebs/claims/${cid}/sources/${cid}`]]) {
