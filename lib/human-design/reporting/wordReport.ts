 ...
 *
 * Bölüm sırası LOCKED (§22): Kapak → İçindekiler → Danışan/Harita → Temel Kimlik →
 * Tanımlı Kanallar → Aktif/Bağımsız Kapılar → Asılı Kapı Bağlamları → Kapanış.
 */
...
export function buildHdReportChildren(
  ...
  // 2) İÇİNDEKİLER
  children.push(...buildTOCPage());

  // 3) DANIŞAN / HARİTA BİLGİLERİ (teknik UUID YOK)
  ...
  if (opts.chartImage && getImgDimensions(opts.chartImage)) {
    ...
    children.push(embedImageParagraph(opts.chartImage, 420));
    children.push(muted("Danışanın Human Design BodyGraph görseli."));
  }

  // 4) TEMEL HUMAN DESIGN KİMLİĞİ
  children.push(h1("Temel Human Design Kimliği", true));
  children.push(...renderIdentity(snapshot.identity.type, "Tip", TYPE_FIELD_ORDER, "Bu haritada tip bilgisi bulunmuyor."));
  ...
  // 7) ASILI KAPI BAĞLAMLARI (kanal içeriğinden; potansiyel kanal başına ayrı)
  ...
  // 8) SADE KURUMSAL KAPANIŞ (AI sentezi YOK; §22)
