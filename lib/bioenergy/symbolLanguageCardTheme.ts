/** Sembol Dili kütüphanesi liste kartı — yumuşak pastel (referans tasarım diliyle hizalı) */
export type SymbolLanguageCardTheme = {
  card: string;
  badge: string;
  badgeMuted: string;
  hover: string;
...
export const SYMBOL_LANGUAGE_CARD_THEMES: SymbolLanguageCardTheme[] = [
  {
...
export function getSymbolLanguageCardTheme(index: number): SymbolLanguageCardTheme {
  return SYMBOL_LANGUAGE_CARD_THEMES[index % SYMBOL_LANGUAGE_CARD_THEMES.length]!;
}
