...
CREATE TABLE IF NOT EXISTS public.cupping_calendar_plans (
  ...
  -- Plan, şablon silinse de yaşar → SET NULL (tenant-safe composite hedef).
  CONSTRAINT cupping_calendar_plans_template_fk
    FOREIGN KEY (tenant_id, advice_template_id)
    REFERENCES public.cupping_advice_templates (tenant_id, id) ON DELETE SET NULL
);
...
CREATE TABLE IF NOT EXISTS public.cupping_client_advice (
  ...
  -- Kaynak şablon silinse de snapshot metin yaşar → SET NULL (provenance temizlenir).
...
-- ─── E) Varsayılan şablon geçişi — ATOMİK (transient çift-varsayılan YOK) ──────
--   partial unique index (yukarıda) nedeniyle "önce hedefi true yap" sırası geçici
--   iki-varsayılana düşebilir. Bu fonksiyon TEK transaction'da ÖNCE diğerlerini
--   temizler, SONRA hedefi varsayılan yapar → invariant eşzamanlı isteklerde de
--   korunur (satır kilidi + tek gövde). En küçük doğru çözüm; büyük soyutlama YOK.
--
...
DECLARE
  v_result jsonb;
BEGIN
  -- 1) Hedef şablon AYNI tenant'a ait olmalı + satır kilidi.
  PERFORM 1
  ...
  -- 2) Diğer TÜM varsayılanları önce kaldır (tek satır kalır → index çakışmaz).
  ...
  -- 3) Hedefi varsayılan + aktif yap (arşivli bir şablon varsayılan yapılamaz →
