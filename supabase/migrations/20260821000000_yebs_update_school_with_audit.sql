-- ============================================================
-- 20260821000000_yebs_update_school_with_audit.sql
--
-- Yaşam Enerjisi Bilgi Sistemi (YEBS) — FAZ API-A1U
-- Atomik SCHOOL UPDATE + AUDIT (partial JSONB patch)
--
-- Amaç: mevcut bir yebs_schools satırının canonical içerik alanlarını güvenli,
...
--
-- Bağlayıcı kararlar (API-A1U):
--   - Tek dış giriş noktası: SECURITY DEFINER RPC
--     public.yebs_update_school_with_audit(...). Fonksiyon owner (tablo sahibi)
...
--   - changed_fields: yalnız gerçekten değişen 5 canonical alan, SABİT sırada;
--     karşılaştırmalar IS DISTINCT FROM (null-safe). updated_at DAHİL DEĞİL.
--   - tradition_id (parent) ve status GÜNCELLENEMEZ (patch whitelist dışı); reparent
--     ve transition bu fazın KAPSAM DIŞIDIR → yeni parent lock/reparent mantığı YOK.
...
--   AUD1 şeması ALTER yok, tablo GRANT'ı genişletilmez. Explicit BEGIN/COMMIT.
--
-- Kararlı hata kodları (ham DB/constraint mesajı veya kullanıcı verisi SIZDIRILMAZ);
--   tümü kontrollü SQLSTATE P0001:
--   YEBS_REQUEST_ID_REQUIRED, YEBS_OPERATION_ID_REQUIRED, YEBS_SCHOOL_ID_REQUIRED,
--   YEBS_EXPECTED_UPDATED_AT_REQUIRED, YEBS_REASON_INVALID, YEBS_INVALID_PATCH,
--   YEBS_ADMIN_NOT_FOUND, YEBS_ADMIN_NOT_ACTIVE, YEBS_SCHOOL_NOT_FOUND,
--   YEBS_SCHOOL_STATUS_LOCKED, YEBS_SCHOOL_STALE_UPDATE, YEBS_SCHOOL_NO_CHANGES,
--   YEBS_SCHOOL_DUPLICATE, YEBS_INVALID_SCHOOL_INPUT.
-- ============================================================

BEGIN;

-- ------------------------------------------------------------
-- Atomik update + audit RPC. Dönüş: canonical public.yebs_schools satırı
...
-- ------------------------------------------------------------
CREATE FUNCTION public.yebs_update_school_with_audit(
...
BEGIN
  ...
  -- --- 6) patch: object, boş değil, yalnız 5 canonical anahtar ---
  -- tradition_id/status/id/actor/request/operation ve unknown key reddedilir.
  ...
  IF v_existing.status IS DISTINCT FROM 'draft' THEN
    RAISE EXCEPTION 'YEBS_SCHOOL_STATUS_LOCKED' USING ERRCODE = 'P0001';
  END IF;
  ...
  -- tradition_id ve status DEĞİŞTİRİLMEZ (patch whitelist dışı; mevcut değer korunur).
  ...
  -- --- Canonical validation (D2 20260726220031 CHECK sözleşmesiyle birebir) ---
  ...
  -- updated_at DAHİL EDİLMEZ. tradition_id/status DEĞİŞMEZ → dahil edilmez.
  ...
  IF cardinality(v_changed) = 0 THEN
    RAISE EXCEPTION 'YEBS_SCHOOL_NO_CHANGES' USING ERRCODE = 'P0001';
  END IF;

  -- --- Canonical UPDATE (yalnız 5 canonical alan; updated_at trigger'la yenilenir) ---
  BEGIN
...
-- (service_role SELECT-only write-gate; A1W create RPC EXECUTE) DEĞİŞTİRİLMEZ.
-- ------------------------------------------------------------
REVOKE ALL ON FUNCTION public.yebs_update_school_with_audit(
...
GRANT EXECUTE ON FUNCTION public.yebs_update_school_with_audit(
  uuid, uuid, uuid, uuid, timestamptz, jsonb, text
) TO service_role;
